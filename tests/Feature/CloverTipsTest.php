...
class CloverTipsTest extends TestCase
{
  ...
  function test_an_authorized_user_can_get_tips_for_specific_employees_for_specific_times() {
    $user = create('App\User');
    $profile = create('App\Profile', ['user_id' => $user->id]);
    ...
    $transactionOfEmployee = create('App\Transaction', ['profile_id' => $profile->id, 'employee_id' => $employeeId, 'status' => 20, 'refund_full' => false, 'pos_transaction_id' => $transactionId, 'created_at' => (Carbon::now())->subDay()]);

    $transactionId0 = "fheuhf3";

    $transactionOfEmployee = create('App\Transaction', ['profile_id' => $profile->id, 'employee_id' => $employeeId, 'status' => 20, 'refund_full' => false, 'pos_transaction_id' => $transactionId0, 'created_at' => (Carbon::now())->subDays(5)]);

    $employeeId1 = 'vfjkd7s';
    $transactionId1 = "bjcsbjvd";

    $transactionOfEmployee1 = create('App\Transaction', ['profile_id' => $profile->id, 'employee_id' => $employeeId1, 'status' => 20, 'refund_full' => false, 'pos_transaction_id' => $transactionId1, 'created_at' => (Carbon::now())->subDay()]);

    $transaction01 = "cbdu478";
    $transactionOfEmployee1 = create('App\Transaction', ['profile_id' => $profile->id, 'employee_id' => $employeeId1, 'status' => 20, 'refund_full' => false, 'pos_transaction_id' => $transaction01, 'created_at' => (Carbon::now())->subDays(6)]);

    $employeeId2 = 'frnj444';
    $transactionId2 = 'cbdhsvhdsv';

    $transactionOfEmployee2 = create('App\Transaction', ['profile_id' => $profile->id, 'employee_id' => $employeeId2, 'status' => 20, 'refund_full' => false, 'pos_transaction_id' => $transactionId2, 'created_at' => (Carbon::now())->subDay()]);
